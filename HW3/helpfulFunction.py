...
def plotTrainCrossEntropyError(trainingError, name="CrossEntropy"):
...
def plotPerplexity(validationPerplexity, name="Perplexity"):
    X = [i for i in range(len(validationPerplexity))]
    # fig1 = plt.figure()
    training, = plt.plot(X, validationPerplexity, '-', label = "Perplexity")
...
def crossEntropy(FX, Y, n=10):
    eY = indicator(Y, n)
    m = len(Y)
    crossEntropy = -1 * np.sum(eY * np.log(FX)) / float(m)
    return crossEntropy

def cross_entropy_one_hot_encoding(fx, y):
    cross_entropy = -1 * np.sum(y*np.log(fx))
    return cross_entropy

def softmax(A):
    # print A
    expA = np.exp(A)
    softmax = expA.T/(np.sum(expA, axis=1))
    softmax = softmax.T
